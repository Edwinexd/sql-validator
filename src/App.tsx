  ...
  const initDb = useCallback(async () => {
    ...
    if (engine === "postgresql") {
      try {
        ...
        // PGlite is a multi-MB WASM bundle loaded on demand; a failed fetch/init must
        // not leave the app stuck with no usable database. Surface it and fall back to
        // the always-available SQLite engine.
        console.error("Failed to initialize the PostgreSQL engine:", e);
        setError(t("engineLoadFailed"));
        setEngine("sqlite");
        return;
      }
    } else {
    ...
    setDbSchema(schema);
    setDatabase(db);
  }, [resetResult, dbData, engine, t, setEngine]);
